...
const ComicsCrawls = (props: Props) => {
	const supabase = useSupabaseClient()

	const [urlParent, setUrlParent] = useState<string>('')
	const [dataCrawls, setDataCrawls] = useState<Array<Comic>>([])
	const [nextPage, setNextPage] = useState<string>('')
	const [dataServer, setDataServer] = useState<Array<ComicServer> | any>([])
	...
	const handleCrawls = () => {
		if (urlParent) {
			...
			axios
				...
				.post(`http://localhost/api/v1/nettruyen/comics`, {
					path: urlParent,
				}, {
					timeout: 20000
				})
				.then((res) => {
					setIsLoading(false)
					setDataCrawls(res.data.data.comics)
	...
	return (
		<>
			<Head>
				<title>Thu Thập Dữ Liệu comics | BCuns</title>
				<SEO
			...
			<main>
				<div className="px-4 sm:px-8 md:px-12 lg:px-16 py-4 md:py-6">
					<div className="flex items-center gap-4 mb-4">
						...
						<h2 className="text-2xl font-bold tracking-tight text-white">Crawls Data comics</h2>
					</div>
					{/* crawls */}
					<div className="text-white mt-10 bg-black/10 p-4 shadow shadow-sky-400 rounded">
						<div>
							...
							<TextareaAutosize
								...
								className="my-2 py-2 font-normal tracking-tight focus:outline-0 resize-none w-full bg-transparent text-white focus:border-b focus:border-sky-500"
						...
						<div className="mb-10">
							<ControlledAccordions
								icon={'https://nettruyentv.net/public/assets/images/logo-nettruyen.png'}
								title="Nettruyen tv"
								handleCallback={handleChangeUrlParentByOpstion}
								childs={[
									{
										title: 'Last Movie',
										url: 'https://nettruyentv.net',
									},
									{
										title: 'Next Page',
										url: nextPage,
